...
fun main(args: Array<String>) {
    ...
    println(TextOperation()
        ...
        .retain(2)
        .insert("s")
        .apply("Lorem ipsum") == "loremums"
    )

    var a = TextOperation(listOf(3, "c", 1))
    var b = TextOperation().retain(3).insert("d").retain(1)
    TextOperation.transform(a, b).let {
//        a = it[0]
//        b = it[1]
        println(it[0].apply(b.apply("aaab")) == it[1].apply(a.apply("aaab")))
    }

    println(a.apply("aaab").length - "aaab".length == a.lenDifference())

    a.apply("aaab")
    println(a.apply("aaab").length - "aaab".length == a.lenDifference())

    var c = TextOperation().retain(3).insert("c").retain(1)
    var s = c.apply("aaab")
    println("aaab" == c.invert(s).apply(s))

    var d1 = TextOperation().retain(3).insert("c").retain(1)
    var d2 = TextOperation().retain(3).insert("c").retain(1)
//    s = d.apply("aaab")
//    var e = TextOperation().retain(3).insert("c").retain(1)
//    d.insert("1")

//    d1 = d1.compose(d2)
    println(d1.apply("aaab"))


    val zdoc = "aaab"
    val za = TextOperation().retain(3).insert("c").insert("c").retain(1)
    val zdoc_a = za.apply(zdoc)
    val zb = TextOperation().retain(3).insert("d").insert("d").retain(1)
    val zab = za.compose(zb)
    println(zb.apply(zdoc_a) == zab.apply(zdoc))
}
